...
namespace UnityPlugin.Decoder
{
	...
	public class MediaDecoder : MonoBehaviour
	{
        ...
        void Update() {
            switch (decoderState) {
                ...
                case DecoderState.BUFFERING:
                    if (NativeDecoder.nativeIsVideoBufferFull(decoderID) || NativeDecoder.nativeIsEOF(decoderID)) {
                        decoderState = DecoderState.START;
                        globalStartTime = AudioSettings.dspTime - hangTime;
                    }
                    break;
		...
		public bool isSeeking() {
			return decoderState >= DecoderState.INITIALIZED && (decoderState == DecoderState.SEEK_FRAME || !NativeDecoder.nativeIsContentReady(decoderID));
		}
		...
		public static void getMetaData(string filePath, out string[] key, out string[] value) {
			...
			int metaCount = NativeDecoder.nativeGetMetaData(filePath, out keyptr, out valptr);
